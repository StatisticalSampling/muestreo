#' @title Función cong.razon
#' @description La función cong.razon es utilizada para estudiar el parámetro razón
#' de los totales poblacionales de X respecto a Y, cuando se realiza un
#' muestreo por conglomerados (unietápico o bietápico).



#' @param numerador: numerador: vector numérico de las observaciones realizadas para la variable, X.
...
#' @param v: vector que contiene los tamaños de las muestras extraídas de cadaconglomerado. (v = 0, representará un muestreo por conglomerados
#'              unietápico).
...
#'de un muestreo por conglomerados unietápico (v = 0; CON1) ó bietápico (CON2):
#'
...
#'  para el parámetro razón poblacional, bajo los supuestos
#'  de normalidad y no normalidad (s.n.-sin normalidad) en la población.


#' @export cong.razon
#' @examples
#'cong.razon(x<-c(rnorm(1000,50,3),rnorm(2000,50,3),rnorm(500,50,3)),
#'y<-c(rnorm(1000,75,4),rnorm(2000,75,4),rnorm(500,75,4)),
#'V=c(1000,2000,500), N=35000, M=10)
...
cong.razon<-function (numerador, denominador, N, M, V, v = 0, alfa = 0.05)
{
  if (v == 0) {
    v <- V
    g <- length(V)
    cat("\n", "CON1(", N, ",", M, ",", g, ")", "\n", "\n")
    congl.razon(numerador, denominador, V, v, N, M, alfa)
  }
  else {
    if (length(v) == length(V)) {
      ...
      cat(")", "\n")
      congl.razon(numerador, denominador, V, v, N, M,
                  alfa)
    }
    else {
